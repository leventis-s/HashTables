/* LinearProbingHashTable takes in strings and a hash function and
 * allocates the data throughout a hash table for easy lookup.
 */
    ...
    if (!contains(elem)) {
        for (int i = 0; i < ourHashFn.numSlots(); i++) {
            ...
            if (elems[index].type != SlotType::FILLED) {
                elems[index] = {elem, SlotType::FILLED};
                logicalSize++;
                return true;
            }
            index++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    ...
    for (int i = 0; i < ourHashFn.numSlots(); i++) {
        ...
        /* Element can't be in table if we hit an empty slot */
