/* RobinHoodHashTable takes in strings and a hash function and
 * allocates the data throughout a hash table using the
 * Robinhood method. It allows for especially easy lookup.
 */
...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ourHashFn = hashFn;
    elems = new Slot[ourHashFn.numSlots()];
    for (int i = 0; i < ourHashFn.numSlots(); i++) {
        elems[i].distance = -1; //Distance is set to -1 for all empty slots
    }
}
...
/* Shifts all the elements after an insertion */
void RobinHoodHashTable::shiftDown(Slot old_elem, int index) {
    /* Loops index back to 0 */
    if (index == ourHashFn.numSlots()) {
        index = 0;
    }
    while (old_elem.distance != 0) { //Ensures it wasn't an empty slot before
        if (old_elem.distance > elems[index].distance) {
            swap(old_elem, elems[index]);
        }
        index++;
        /* Loops index back to 0 */
        if (index == ourHashFn.numSlots()) {
...
bool RobinHoodHashTable::insert(const string& elem) {
    int index = ourHashFn(elem);
    /* Ensure there is space and elem is not already in array */
    if (logicalSize != ourHashFn.numSlots() && !contains(elem)) {
        for (int distance = 0; distance < ourHashFn.numSlots(); distance++) {
            /* Loops index back to 0 */
            if (index == ourHashFn.numSlots()) {
               index = 0;
            }
            /* Finds place to insert elem */
            if (distance > elems[index].distance) {
                Slot old_elem = {elems[index].value, elems[index].distance + 1}; //Old element has an increased distance as it moves
                shiftDown(old_elem, index + 1);
                elems[index] = {elem, distance};
                logicalSize++;
                return true;
            }
            index++;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for (int distance = 0; distance < ourHashFn.numSlots(); distance++) {
        ...
        if (elems[index].value == elem && elems[index].distance != -1) {
            return true;
        }
        /* Stops looking if its distance is larger than current element's distance */
        if (distance > elems[index].distance) {
            return false;
        }
        index++;
...
/* Shifts elements after a removal */
void RobinHoodHashTable::shiftUp(int index) {
    int nextIndex = index + 1;
    /* Loops index back to 0 */
    if (nextIndex == ourHashFn.numSlots()) {
        nextIndex = 0;
    }
    while (elems[nextIndex].distance > 0 && elems[index].distance == -1) {
        elems[nextIndex].distance--; //Decreases distance since it's moving closer to desired spot
        swap(elems[nextIndex], elems[index]);
        index = nextIndex;
        nextIndex++;
        /* Loops back to 0 */
        if (nextIndex == ourHashFn.numSlots()) {
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    if (contains(elem)) {
        ...
        elems[index].distance = -1; //Sets the distance to -1 because spot is now empty
        logicalSize--;
        shiftUp(index);
        return true;
    }
...
void RobinHoodHashTable::printDebugInfo() const {
    /* TODO: Remove this comm
     * ent and implement this function. */
}
